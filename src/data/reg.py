...
__all__ = ["RegDataModule", "RegDataset"]
    

class RegDataModule(lightning.LightningDataModule):
    def __init__(
        self, 
        ...
        **kwargs
        ...
        self.y_load_path = os.path.join(data_load_fold, 'y.pt')
    ...
    def setup(self, stage=None) -> None:
        ...
        y = torch.load(self.y_load_path, weights_only=True)     # (N, ...)
        ...
        # dataset
        self.train_dataset = RegDataset(
            x[sample[self.split] == 0], y[sample[self.split] == 0],
            channel_perm=self.channel_perm, 
            channel_drop=self.channel_drop,
            channel_shift=self.channel_shift,
        )
        self.val_dataset   = RegDataset(
            x[sample[self.split] == 1], y[sample[self.split] == 1]
        )
        self.test_dataset  = RegDataset(
            x[sample[self.split] == 2], y[sample[self.split] == 2]
        )
...
class RegDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i: int) -> tuple[torch.Tensor, ...]:
        ...
        # augment x
        x, x_channel_idx = RegDataset.augment(
            x, x_channel_idx, 
            channel_perm=self.channel_perm, 
            channel_drop=self.channel_drop, 
            channel_shift=self.channel_shift
